...
class SistemaEventos:
    ...
    def cadastrar_evento(self, nome=None, data_str=None, hora=None, local=None,
        ...
        elif tipo == "3": # Workshop
            while not materiais:
                materiais = input("Digite os materiais necessários para o workshop: ").strip()
                if not materiais:
                    print(f"{Cores.AMARELO}⚠️ A lista de materiais é obrigatória.{Cores.RESET}")
            while not instrutor:
                instrutor = input("Digite o nome do instrutor: ").strip()
                if not instrutor:
                    print(f"{Cores.AMARELO}⚠️ O nome do instrutor é obrigatório.{Cores.RESET}")
                    

        # ====== Criação do objeto ======
        if tipo == "3":
            evento = Workshop(nome, data, hora_obj, local, capacidade, categoria, preco, materiais, instrutor)
        elif tipo == "2":
    ...
    # ============================================================
    def fazer_checkin_participante(self, indice_evento, usuario):
        """Realiza o check-in de um participante em um evento, se for o dia correto."""
        try:
            evento = self.eventos[indice_evento - 1]

            # PASSO 2: Adiciona a verificação de data antes de tudo
            if not evento.is_checkin_disponivel():
                return False, f"❌ Check-in para '{evento.get_nome()}' só está disponível no dia {evento.get_data()}."

            # Lógica do check-in
            for p in evento.get_participantes():
                if p.get_email() == usuario.get_email():
                    p.fazer_checkin()
                    self.salvar()
                    return True, f"✅ Check-in realizado com sucesso no evento {evento.get_nome()}!"
            
            return False, "❌ Usuário não encontrado na lista de participantes deste evento."
